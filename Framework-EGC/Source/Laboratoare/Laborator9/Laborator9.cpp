...
void Laborator9::Update(float deltaTimeSeconds)
{
...

	{
		...
		RenderSimpleMesh(meshes["box"], shaders["ShaderLab9"], modelMatrix, mapTextures["crate"], mapTextures["bamboo"]);
	}
...
void Laborator9::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, Texture2D* texture1, Texture2D* texture2)
{
	...
	auto loc_camera_x = glGetUniformLocation(shader->program, "camera_x");
	if (mesh == meshes["square"])
		 glUniform1f(loc_camera_x, 0*GetSceneCamera()->GetFieldOfViewX());
	else glUniform1f(loc_camera_x, 0);

	auto loc_camera_y = glGetUniformLocation(shader->program, "camera_y");
	if (mesh == meshes["square"])
		 glUniform1f(loc_camera_y, GetSceneCamera()->GetFieldOfViewY());
	else glUniform1f(loc_camera_y, 0);
