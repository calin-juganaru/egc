...
void Laborator3_Vis2D::OnInputUpdate(float deltaTime, int mods)
{
	...
	{
		this->logicSpace.x += 0.1f;
	}
	else if (glfwGetKey(window->GetGLFWWindow(), GLFW_KEY_D) == GLFW_PRESS)
	{
		this->logicSpace.x -= 0.1f;
	}
	else if (glfwGetKey(window->GetGLFWWindow(), GLFW_KEY_Z) == GLFW_PRESS)
	{
		this->logicSpace.width *= 1.25f;
		this->logicSpace.height *= 1.25f;
	}
	else if (glfwGetKey(window->GetGLFWWindow(), GLFW_KEY_X) == GLFW_PRESS)
	{
		this->logicSpace.width /= 1.25f;
		this->logicSpace.height /= 1.25f;
	}
