...
void Laborator7::Update(float deltaTimeSeconds)
{
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["ShaderLab7"], modelMatrix, glm::vec3(1, 0, 0));
	}
...
void Laborator7::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color)
{
	...
	auto light_pos = glGetUniformLocation(shader->program, "light_position");
	...
	auto aux = glGetUniformLocation(shader->program, "material_shininess");
	...
	glUniform3fv(aux, 1, glm::value_ptr(color));
